...
class App_normal(tk.Tk):
    ...
    def call_next(self):
        if self.queue_listbox.size() != 0:
            self.engine.say(f"{self.queue_listbox.get(0)}, приготовьтесь")
            self.curr_client_label.configure(text=f"{self.queue_listbox.get(0)}")
    ...
    def get_ticket(self):
        ...
        self.queue_listbox.insert(tk.END, f"Клиент {self.click + str(ticket_number)}")
    ...
    def start_timer(self):
        ...
        self.after(100000, self.start_timer)
